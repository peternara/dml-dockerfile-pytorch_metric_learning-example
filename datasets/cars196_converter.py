...
if __name__ == "__main__":
    ...
    for i, filename in tqdm(
        enumerate(jpg_filenames), total=num_examples, desc=hdf5_filepath
    ):
        raw_image = cv2.imread(
            os.path.join(fuel_data_path, filename), cv2.IMREAD_COLOR
        )  # BGR image
    ...
    split_dict = dict(
        train=dict(images=split_train, targets=split_train),
